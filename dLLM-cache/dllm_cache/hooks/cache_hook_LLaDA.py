...
def cache_hook_feature(
    ...
    # if self.layer_id == 10: # Log only for layer 10 to avoid spam
    #     print(f"[Step {feature_cache.current_step}] "
    #           f"Refresh Gen: {refresh_gen}, "
    #           f"Refresh Prompt: {refresh_prompt}, "
    #           f"Transfer Active: {transfer}")
    ...
    if hasattr(feature_cache, 'step_logs'):
        step = feature_cache.current_step
        if step not in feature_cache.step_logs:
            feature_cache.step_logs[step] = {}
        feature_cache.step_logs[step][self.layer_id] = {
